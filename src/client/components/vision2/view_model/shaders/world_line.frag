...
void main() {
  ...
  // Get the radius of the curve we are drawing
  float radius = dot(axis, start);

  // Project it into the world space
  vec3 cam = unproject(screenPoint, focalLength * viewSize.x, centre, projection);

  // Rotate the axis vector towards the screen point by the angle to radius
  // This gives the closest point on the curve
  vec3 nearestPoint = rotateByAxisAngle(axis, normalize(cross(axis, cam)), acos(radius));
