...
export class FakeNodeClock implements Clock {
  ...
  public setImmediate(fn: () => void): () => void {
    const id = this.nextId++
    this.addTask({ id, nextTime: this.now() + 1, fn })
    return () => this.removeTask(id)
  }
  ...
  public runOnlyPendingTimers() {
    ...
    while (this.tasks.length > 0 && this.now() <= this.tasks[this.tasks.length - 1].nextTime) {
      if (i > limit) {
        throw new Error(`Exceeded clock task limit of ${limit}, possibly caused by a infinite task loop?`)
      }
      const task = this.tasks[0]
